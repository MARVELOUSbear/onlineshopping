									...
									try{
										  ...
										  if(rs.next())
										  {
										    String Id = rs.getString(1);
										  	PreparedStatement pswl=con.prepareStatement("SELECT * FROM orderlist_"+Id);
										  	ResultSet rswl= pswl.executeQuery();
										  	while (rswl.next()){
										  		out.println("<form action=\"/OnlineShopping/ConfirmOrder\" method=\"POST\">");
												out.println("	<tr>");
										  		out.println("	<input type=\"hidden\" name=\"hidden\" value=\""+rswl.getString(1)+"\" />");
						                        out.println("    <td>"+rswl.getString(2)+"</td>");
						                        out.println("    <td>"+rswl.getString(4));
						                        out.println("        <small>item(s)</small>");
						                        out.println("    </td>");
						                        out.println("    <td>"+rswl.getString(5)+"</td>");
						                        out.println("    <td data-value=\"78025368997\">"+rswl.getString(3)+"</td>");
												out.println("    <td>"+rswl.getString(6)+"</td>");
						                        out.println("    <td class=\"delete\"><button type=\"submit\" class=\"btn   btn-primary\" > Confirm</button></td>");
						                        out.println("	</tr>");
												out.println("</form>");
